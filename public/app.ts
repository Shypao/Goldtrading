...
function handleBuyingCustomerChoice(){
  ...
  if(sellerInput) sellerInput.value=enteringNew?(newCustomerInput?.value||''):choice;
  if(enteringNew) setTimeout(()=>newCustomerInput?.focus(),0);
  scheduleBuyingDraftSave();
}
function syncBuyingCustomerName(){
  const sellerInput=document.getElementById('b_seller_name');
  if(sellerInput) sellerInput.value=val('b_new_customer_name');
  scheduleBuyingDraftSave();
}
