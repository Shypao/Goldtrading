...
async function syncSharedPricing(){
  ...
  try{
    ...
    if(response.status===401){ showLogin(); return; }
    const result=await response.json().catch(()=>null);
    if(!response.ok) throw new Error(result?.error||'Pricing sync failed');
    if(Number.isInteger(result?.revision)) db._revision=result.revision;
    if(!result?.pricing||JSON.stringify(result.pricing)===previousPricing) return;
...
async function commitPurchaseBatch(printAfter=false){
  ...
  if(customer.isNew){ customerId=uid('cust');newCustomer={id:customerId,name:customer.name,contact:'',notes:''};db.customers.push(newCustomer); }
...
function purchaseReceiptMarkup(items){
  ...
  const itemLines=items.map(item=>`<tr class="receipt-item"><td class="receipt-description">${esc(item.metal)} ${esc(gradeLabel(item.metal,item.karat))} · ${esc(item.itemType)}<small class="receipt-item-rate">Rate: PHP ${receiptMoneyNumber(item.rate)}/g</small></td><td class="receipt-qty">1</td><td class="receipt-weight">${receiptWeightNumber(item.netWeight)}g</td><td class="receipt-amount">${receiptMoneyNumber(item.payout)}</td></tr>`).join('');
...
function printPurchaseReceipt(batchId){
  ...
  if(!printWindow){toast('Allow the receipt print window, then try again');return;}
...
function toggleInventoryForLiquidation(id,checked){
  ...
  if(poolButton){const poolItems=selectedInventoryForPool();poolButton.disabled=poolItems.length<2||new Set(poolItems.map(item=>item.metal)).size!==1||new Set(poolItems.map(item=>item.karat)).size!==1;}
...
function openManualInventoryPoolModal(){
  ...
  if(items.length<2){toast('Select at least two unpooled inventory items');return;}
...
async function createManualInventoryPool(){
  const items=selectedInventoryForPool();
  if(items.length<2){closeInventoryPoolModal();toast('The selected inventory changed. Select at least two items again.');return;}
  ...
  items.forEach(item=>{item.inventoryPoolId=id;});db.inventoryPools.push(pool);syncInventoryPool(pool);
  if(!await saveDB()){db=beforeState;render();toast('The inventory pool was not created');return;}
  inventoryMoveSelection.clear();closeInventoryPoolModal();render();toast(`${id} created from ${items.length} selected items`);
}
async function toggleInventoryPoolHold(id){
  ...
  const before=pool.onHold;pool.onHold=!pool.onHold;pool.updatedAt=new Date().toISOString();syncInventoryPool(pool);
  if(!await saveDB()){pool.onHold=before;syncInventoryPool(pool);render();return;}render();toast(`${pool.id} is now ${pool.status}`);
}
...
function openPoolLiquidationModal(id,initialType='partial'){
  ...
  const entire=initialType==='entire';
...
async function confirmPoolLiquidation(){
  ...
  if(!prepared){toast(`Enter a weight between 0.01 g and ${snapshot.weight.toFixed(2)} g`);return;}if(proceeds<=0){toast('Enter the selling price / proceeds');return;}if(!buyer){toast('Enter the buyer / refiner');return;}
  prepared.poolItems=snapshot.items;const beforeState=JSON.parse(JSON.stringify(db));if(!applyPooledInventoryAllocation(prepared)){toast('The pool balance changed. Review it and try again.');return;}
  let assignedProceeds=0;const lines=prepared.allocations.map((line,index)=>{const source=db.stock.find(item=>item.id===line.itemId);const amount=index===prepared.allocations.length-1?roundMoney(proceeds-assignedProceeds):roundMoney(proceeds*(line.weight/prepared.weight));assignedProceeds=roundMoney(assignedProceeds+amount);return {...line,metal:source?.metal||pool.metal,assay:source?.karat||pool.karat,costPortion:line.cost,sellingAmount:amount,proceeds:amount,sellingRate:roundMoney(amount/line.weight)};});
  const remaining=syncInventoryPool(pool),id=nextSequenceId('L',db.liquidations),recordedAt=new Date().toISOString();pool.updatedAt=recordedAt;
  ...
  if(!await saveDB()){db=beforeState;render();toast('The pool liquidation was not recorded');return;}closePoolLiquidationModal();render();toast(`${id} recorded · ${fmtWeight(prepared.weight)} liquidated`);
}
...
function updateInventoryItemLiquidationPreview(){
  const item=db.stock.find(stock=>stock.id===val('inventory_item_liquidation_id'));if(!item)return;
  const entire=val('inventory_item_liquidation_type')==='entire',input=document.getElementById('inventory_item_liquidation_weight');
  if(input){input.disabled=entire;if(entire)input.value=String(item.currentWeight);}
  ...
  Object.entries(values).forEach(([id,value])=>{const element=document.getElementById(id);if(element)element.textContent=value;});
  const button=document.getElementById('create_inventory_item_liquidation');if(button)button.disabled=!prepared;
}
function liquidateInventoryItem(id){
  ...
  const modal=document.createElement('div');modal.id='inventory_item_liquidation_modal';modal.className='modal-backdrop';
  ...
  modal.addEventListener('click',event=>{if(event.target===modal)closeInventoryItemLiquidationModal();});document.body.appendChild(modal);updateInventoryItemLiquidationPreview();document.getElementById('inventory_item_liquidation_weight')?.focus();
}
...
function openInventoryEdit(id){
  ...
  if(item.inventoryPoolId){toast(`Manage this item through ${item.inventoryPoolId}`);return;}
...
async function submitLiquidationBatch(batchId){
  ...
  if(typeof location!=='undefined'&&(location.protocol==='http:'||location.protocol==='https:')){
    ...
    if(!result)return;
    closeCompleteLiquidationBatch();render();toast(`Liquidation ${result.liquidation.id} recorded`);return;
  }
  const beforeState=JSON.parse(JSON.stringify(db));
...
async function saveCompletedLiquidationBatch(batch,details){
  try{
    ...
    for(const updated of result.updatedItems||[]){
      const item=db.stock.find(stock=>stock.id===updated.id);if(item)Object.assign(item,updated);
    }
...
function prepareInventoryForRefining(){
  ...
  refMetal=metals[0]; refiningSelection.clear(); selected.forEach(item=>refiningSelection.add(item.id)); inventoryMoveSelection.clear(); goTab('refining');
