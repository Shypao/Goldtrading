...
function changeInventoryWeek(delta){
  clearInventoryLiquidationSelection(); inventoryWeekOffset+=delta;
...
function toggleInventoryForLiquidation(id,checked){
  const item=db.stock.find(stock=>stock.id===id); if(!item||!selectableInventory(item)) return;
  if(checked) liquidationSelection.add(id); else { liquidationSelection.delete(id); liquidationDraft.delete(id); }
...
function moveInventorySelectionToLiquidation(percentage){
  ...
  if(![25,50,75,100].includes(portion)){ toast('Choose 25%, 50%, 75%, or 100%'); return; }
  ...
  if(selected.length!==required){ toast(`Select exactly ${required} of ${pool.length} stock records to move ${portion}%`); return; }
  ...
  liqMetal=metals[0]; liqKarat='All'; liqStatus='All';
...
function renderInventory(){
  ...
  <section class="block">
    ...
    ${tableOrEmpty(rows, s=>`<tr>${isAdmin()?`<td><input type="checkbox" aria-label="Select ${esc(s.metal)} ${esc(s.karat)} from ${esc(s.customerName)}" onchange="toggleInventoryForLiquidation('${s.id}',this.checked)" ${liquidationSelection.has(s.id)?'checked':''} ${selectableInventory(s)?'':'disabled'}></td>`:''}<td>${fmtDate(s.date)}</td><td>${esc(s.customerName)}</td><td><span class="metal-tag ${s.metal.toLowerCase()}">${s.metal}</span> ${esc(s.karat)}</td>
      ...
      [...(isAdmin()?['Select']:[]),'Date','Customer','Metal / karat','Type','Current weight','Cost','Status','Location','Remarks',...(isAdmin()?['Actions']:[])],
...
function renderLiquidation(){
  ...
  <section class="block">
    ...
    ${tableOrEmpty(db.liquidations.slice().sort((a,b)=>b.date.localeCompare(a.date)),
      ...
      ['Date','Metal','Buyer / refiner','Released wt','Selling prices','Proceeds','Payment','Cost','Margin',...(isAdmin()?['Actions']:[])],
...
function liquidationRateLabel(record){
  ...
  (record.lines||[]).forEach(line=>{
    ...
    if(rate>0&&!rates.includes(label)) rates.push(label);
  });
  if(!rates.length&&Number(record.sellingRate)>0) rates.push(`${fmtMoney(record.sellingRate)}/g`);
...
function submitLiquidation(){
  ...
  for(const row of rows){
    ...
    if(rate<=0){ toast(`Enter a selling price for ${item.karat}`); return; }
  ...
  for(const entry of prepared){
    ...
    totalWeight += w; totalCost += costPortion; totalProceeds += lineProceeds;
...
async function saveLiquidationEdit(){
  ...
  if((record.lines||[]).length){
    ...
    for(let index=0;index<record.lines.length;index++){
      ...
      if(!Number.isFinite(rate)||rate<=0){ toast(`Enter a valid selling price for ${line.assay||'each item'}`); return; }
    ...
    if(!Number.isFinite(rate)||rate<=0){ toast('Enter a valid selling price'); return; }
    record.sellingRate=roundMoney(rate); record.proceeds=roundMoney(Number(record.releasedWeight)*rate);
  }
