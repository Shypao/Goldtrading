...
function applyPooledInventoryAllocation(prepared) {
    ...
    if (resolved.some(({ line, item }) => !item || !movableInventory(item) || Number(item.currentWeight) + 0.005 < Number(line.weight)))
        return false;
...
function openPooledLiquidationModal() {
    ...
    if (!metals.length) {
        toast('No available Gold or Silver inventory can be pooled');
        return;
    }
    ...
    const modal = document.createElement('div');
    modal.id = 'pooled_liquidation_modal';
    modal.className = 'modal-backdrop';
    ...
    document.body.appendChild(modal);
    updatePooledLiquidationPreview();
    document.getElementById('pooled_liquidation_weight')?.focus();
}
async function createPooledLiquidationBatch() {
    ...
    if (!prepared) {
        toast(`Enter a weight between 0.01 g and ${pool.weight.toFixed(2)} g`);
        return;
    }
    if (!buyer || !name) {
    ...
    if (!applyPooledInventoryAllocation(prepared)) {
        toast('The inventory pool changed. Review it and try again.');
        return;
    }
    ...
    if (!await saveDB()) {
        db = beforeState;
        render();
        toast('The pooled liquidation batch was not created');
        return;
    }
    closePooledLiquidationModal();
    goTab('liquidation');
