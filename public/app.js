...
async function saveGradeFormula(event) {
    ...
    if (db.pricing.dailyFormula.effectiveDate !== todayStr())
        db.pricing.dailyFormula = { effectiveDate: todayStr(), multipliers: {} };
    ...
    await saveDB();
    render();
    toast(`${metal} ${gradeLabel(metal, key)} formula updated for today`);
}
async function resetGradeFormula() {
    if (!formulaEditTarget || !adminEditGuard())
        return;
    const { metal, key } = formulaEditTarget;
    if (db.pricing.dailyFormula?.multipliers?.[metal])
        delete db.pricing.dailyFormula.multipliers[metal][key];
    closeGradeFormulaEditor();
    await saveDB();
    render();
    toast(`${metal} ${gradeLabel(metal, key)} formula reset for today`);
}
