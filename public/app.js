...
function renderLiquidation() {
    ...
    if (!batches.some(batch => batch.id === activeLiquidationBatchId))
        activeLiquidationBatchId = batches[0]?.id || '';
    ...
    <div class="item-check-wrap liquidation-items">
      ...
      ${lines.map(line => {
        ...
        return `<div class="item-check-row" data-item="${esc(line.itemId)}">
...
function updateInlineLiquidationPreview() {
    const batch = db.liquidationBatches.find(record => record.id === activeLiquidationBatchId);
    if (!batch)
        return;
    ...
    if (proceeds)
        proceeds.textContent = offer ? fmtMoney(offer) : 'Not entered';
    if (margin) {
        margin.textContent = fmtMoney(difference);
        margin.style.color = difference >= 0 ? 'var(--sage)' : 'var(--rust)';
    }
    if (marginPct)
        marginPct.textContent = `${cost ? (difference / cost * 100).toFixed(2) : '0.00'}% profit margin`;
}
async function saveInlineLiquidationBatch(openSale = false) {
    ...
    render();
    if (openSale)
        openCompleteLiquidationBatch(batch.id);
    else
...
async function returnLiquidationBatchItem(batchId, itemId) {
    ...
    if (!batch || !line || !item || !adminEditGuard())
        return;
    if (!confirm(`Return ${item.metal} ${gradeLabel(item.metal, item.karat)} to Current Inventory?`))
        return;
    ...
    if (!await saveDB()) {
        db = beforeState;
        render();
        toast('The item was not returned');
        return;
    }
    render();
    toast('Item returned to Current Inventory');
}
...
async function returnLiquidationBatch(id) {
    ...
    if (linked.some(({ item }) => !item || item.status !== 'For Liquidation' || item.liquidationBatchId !== batch.id)) {
