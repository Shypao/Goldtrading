...
function renderBuying() {
  ...
  <section class="block recent-purchases">
    ...
    ${tableOrEmpty(db.stock.filter(s => !s.sourceRefiningBatchId).slice().sort((a, b) => b.date.localeCompare(a.date)).slice(0, 10), s => `<tr><td data-label="Date">${fmtDate(s.date)}</td><td data-label="Customer">${esc(s.customerName)}</td><td data-label="Metal / karat"><span class="metal-tag ${s.metal.toLowerCase()}">${s.metal}</span> ${esc(s.karat)}</td>
