...
function sessionUser(request) {
    ...
    const account = database.prepare('SELECT username, display_name AS displayName, role, active FROM users WHERE id = ?').get(session.user.id);
    if (!account?.active) {
        sessions.delete(token);
        return null;
    }
...
function isLedgerState(value) {
    if (!value || typeof value !== 'object')
        return false;
    const candidate = value;
    return Object.keys(tableMap).every(key => Array.isArray(candidate[key]));
}
function unchangedRecord(candidate, original) {
    return JSON.stringify(candidate) === JSON.stringify(original);
}
function saveStaffAdditions(candidate) {
    const current = loadState();
    if (!candidate.pricing || !current.pricing)
        throw new Error('Pricing settings are unavailable');
    const candidatePricing = JSON.parse(JSON.stringify(candidate.pricing));
    const currentPricing = JSON.parse(JSON.stringify(current.pricing));
    const permittedGrades = {
    ...
    const nextPricing = JSON.parse(JSON.stringify(current.pricing));
    for (const metal of Object.keys(permittedGrades))
        nextPricing[metal].overrides = requestedOverrides[metal];
    ...
    for (const customer of newCustomers) {
        if (!customer.id || !String(customer.name ?? '').trim())
    ...
    try {
        ...
        for (const record of newCustomers)
            insertCustomer.run(record.id, JSON.stringify(record));
        for (const record of newStock)
            insertStock.run(record.id, JSON.stringify(record));
...
function loadState() {
    ...
    const pricingRow = database.prepare("SELECT value FROM settings WHERE key = 'pricing'").get();
    state.pricing = pricingRow ? JSON.parse(pricingRow.value) : null;
    return state;
}
    ...
    try {
        for (const [key, table] of Object.entries(tableMap)) {
            ...
            for (const record of state[key]) {
                if (!record.id)
                    throw new Error(`${key} contains a record without an id`);
...
async function createMarketProposal(payoutPercentage) {
    ...
    if (!usdPhp || Object.values(spotUsd).some(value => !value))
    ...
    const perGram = (price) => +(price * usdPhp / gramsPerTroyOunce * factor).toFixed(2);
    const now = new Date();
...
function applyMarketProposal(proposal) {
    const state = loadState();
    if (!state.pricing)
        return;
...
const server = http.createServer(async (request, response) => {
    ...
    try {
        if (request.method === 'GET' && url.pathname === '/api/health')
            return sendJson(response, 200, { ok: true, database: databaseFile });
        ...
        if (url.pathname.startsWith('/api/') && !user)
            return sendJson(response, 401, { error: 'Sign in required' });
        if (request.method === 'GET' && url.pathname === '/api/state')
            return sendJson(response, 200, publicStateFor(user));
        if (request.method === 'PUT' && url.pathname === '/api/state') {
            const body = await readJsonBody(request);
            if (!isLedgerState(body))
                return sendJson(response, 400, { error: 'Invalid ledger state' });
            if (user.role === 'admin')
                saveState(body);
            else {
                try {
                    saveStaffAdditions(body);
                }
                catch (error) {
                    return sendJson(response, 403, { error: error instanceof Error ? error.message : 'Staff action is not permitted' });
                }
            }
        ...
        if (request.method === 'GET' && url.pathname === '/api/market') {
            ...
            if (url.searchParams.get('apply') === '1')
                applyMarketProposal(proposal);
            return sendJson(response, 200, proposal);
        }
        if (request.method === 'GET' && url.pathname === '/api/users') {
            if (user.role !== 'admin')
                return sendJson(response, 403, { error: 'Administrator access required' });
        ...
        if (request.method === 'POST' && url.pathname === '/api/users') {
            ...
            if (!/^[a-z0-9._-]{3,40}$/.test(username))
                return sendJson(response, 400, { error: 'Username must be 3–40 letters, numbers, dots, dashes, or underscores' });
            if (!displayName)
                return sendJson(response, 400, { error: 'Display name is required' });
            if (password.length < 8)
                return sendJson(response, 400, { error: 'Password must be at least 8 characters' });
            if (!['admin', 'staff'].includes(role))
                return sendJson(response, 400, { error: 'Role must be Admin or Staff' });
            try {
        ...
        FROM users WHERE id = ?`).get(targetId);
            ...
            if (!['admin', 'staff'].includes(role))
                return sendJson(response, 400, { error: 'Role must be Admin or Staff' });
            if (active < 0)
                return sendJson(response, 400, { error: 'Account status is required' });
            if (password && password.length < 8)
                return sendJson(response, 400, { error: 'New password must be at least 8 characters' });
            ...
            if (target.role === 'admin' && target.active && (role !== 'admin' || !active)) {
                const activeAdminCount = Number(database.prepare("SELECT COUNT(*) AS count FROM users WHERE role = 'admin' AND active = 1").get().count);
                if (activeAdminCount <= 1)
                    return sendJson(response, 400, { error: 'At least one active administrator is required' });
            }
            if (password) {
                const salt = randomBytes(16).toString('hex');
        ...
        if (request.method === 'DELETE' && userEditMatch) {
            if (user.role !== 'admin')
                return sendJson(response, 403, { error: 'Administrator access required' });
            const targetId = decodeURIComponent(userEditMatch[1]);
            const target = database.prepare('SELECT id, username, role, active FROM users WHERE id = ?').get(targetId);
            if (!target)
                return sendJson(response, 404, { error: 'Account not found' });
            if (target.id === user.id)
                return sendJson(response, 400, { error: 'You cannot delete the account currently signed in' });
